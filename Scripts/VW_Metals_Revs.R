...
### Updated 15 May 2020 - used Bathy data from 2018 acoustic scans for FCR and BVR

## Saved RFile as: VW_Metals_Revs
...
depth_fcr <- read_csv("C:/Users/ahoun/OneDrive/Desktop/GHG/Data/FCR_Bathy_Acoustic.csv")

# BVR Volumes by depth
depth_bvr <- read_csv("C:/Users/ahoun/OneDrive/Desktop/GHG/Data/BVR_Bathy_Acoustic.csv")
...
for(i in 1:length(fcr_tfe_layers$DateTime)){
  hypo_fcr_tfe[i] <- sum(fcr_tfe_layers$FCR_6[i]*depth_fcr$Vol_m3[6],
                          fcr_tfe_layers$FCR_7[i]*depth_fcr$Vol_m3[7])/sum(depth_fcr$Vol_m3[c(6:7)])
}

hypo_fcr_tmn <- rep(-99,length(fcr_tmn_layers$DateTime))
for(i in 1:length(fcr_tmn_layers$DateTime)){
  hypo_fcr_tmn[i] <- sum(fcr_tmn_layers$FCR_6[i]*depth_fcr$Vol_m3[6],
                         fcr_tmn_layers$FCR_7[i]*depth_fcr$Vol_m3[7])/sum(depth_fcr$Vol_m3[c(6:7)])
}

hypo_fcr_sfe <- rep(-99,length(fcr_sfe_layers$DateTime))
for(i in 1:length(fcr_sfe_layers$DateTime)){
  hypo_fcr_sfe[i] <- sum(fcr_sfe_layers$FCR_6[i]*depth_fcr$Vol_m3[6],
                         fcr_sfe_layers$FCR_7[i]*depth_fcr$Vol_m3[7])/sum(depth_fcr$Vol_m3[c(6:7)])
}

hypo_fcr_smn <- rep(-99,length(fcr_smn_layers$DateTime))
for(i in 1:length(fcr_smn_layers$DateTime)){
  hypo_fcr_smn[i] <- sum(fcr_smn_layers$FCR_6[i]*depth_fcr$Vol_m3[6],
                         fcr_smn_layers$FCR_7[i]*depth_fcr$Vol_m3[7])/sum(depth_fcr$Vol_m3[c(6:7)])
}
...
ggarrange(fe_16,fe_17,fe_18,common.legend=FALSE,ncol=2,nrow=2)
# Saved as: W = 800, H = 700
...
ggarrange(mn_16,mn_17,mn_18,common.legend=FALSE,ncol=2,nrow=2)
