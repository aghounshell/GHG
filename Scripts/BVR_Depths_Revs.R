...
###   Updated GHG Database from RPM
###   Updated Bathy data from 2018 acoustic scans

# RFile: VW_BVR_16to18_Revs
...
depth <- read_csv("C:/Users/ahoun/OneDrive/Desktop/GHG/Data/BVR_Bathy_Acoustic.csv")
...
casts_1 <- casts %>% select(Date,Depth_m,Temp_C,DO_mgL) %>% filter(Depth_m>=0 & Depth_m<2) %>% group_by(Date) %>% 
  summarize_all(funs(mean)) %>% arrange(Date) %>% mutate(grouping="BVR_1")

casts_2 <- casts %>% select(Date,Depth_m,Temp_C,DO_mgL) %>% filter(Depth_m>=2 & Depth_m<5) %>% group_by(Date) %>% 
  summarize_all(funs(mean)) %>% arrange(Date) %>% mutate(grouping="BVR_2")

casts_3 <- casts %>% select(Date,Depth_m,Temp_C,DO_mgL) %>% filter(Depth_m>=5 & Depth_m<8) %>% group_by(Date) %>% 
  summarize_all(funs(mean)) %>% arrange(Date) %>% mutate(grouping="BVR_3")

casts_4 <- casts %>% select(Date,Depth_m,Temp_C,DO_mgL) %>% filter(Depth_m>=8) %>% group_by(Date) %>% 
  summarize_all(funs(mean)) %>% arrange(Date) %>% mutate(grouping="BVR_4")

# Re-combine all casts then use spread to convert to wide form
casts_merge <- rbind(casts_1,casts_2,casts_3,casts_4)
...
# Plot averaged temperature for 'casts_4' (aka: > 8 m)
ggplot(casts_layers,aes(x=Date,y=BVR_4))+
  geom_line(aes(x=Date,y=BVR_4))+
  geom_point()+
  theme_classic()
...
write_csv(swi_16, path = "C:/Users/ahoun/OneDrive/Desktop/GHG/Data_Output/swi_16_bvr_revs.csv")
write_csv(swi_17, path = "C:/Users/ahoun/OneDrive/Desktop/GHG/Data_Output/swi_17_bvr_revs.csv")
write_csv(swi_18, path = "C:/Users/ahoun/OneDrive/Desktop/GHG/Data_Output/swi_18_bvr_revs.csv")

# Plot all years on same graph
ggplot()+
  geom_line(data=swi_16,aes(x=doy,y=BVR_4,color="2016"),size=1.1)+
  geom_point(data=swi_16,aes(x=doy,y=BVR_4,color="2016"),size=4)+
  geom_line(data=swi_17,aes(x=doy,y=BVR_4,color="2017"),size=1.1)+
  geom_point(data=swi_17,aes(x=doy,y=BVR_4,color="2017"),size=4)+
  geom_line(data=swi_18,aes(x=doy,y=BVR_4,color="2018"),size=1.1)+
  geom_point(data=swi_18,aes(x=doy,y=BVR_4,color="2018"),size=4)+
...
for(i in 1:length(casts_layers$Date)){
  vw_temp[i] <- sum(casts_layers$BVR_1[i]*depth$Vol_m3[1],
                    casts_layers$BVR_2[i]*depth$Vol_m3[2],
                    casts_layers$BVR_3[i]*depth$Vol_m3[3],
                    casts_layers$BVR_4[i]*depth$Vol_m3[4])/sum(depth$Vol_m3[c(1:4)])
}
...
for(i in 1:length(casts_layers$Date)){
  vw_epi_temp[i] <- sum(casts_layers$BVR_1[i]*depth$Vol_m3[1],
                    casts_layers$BVR_2[i]*depth$Vol_m3[2])/sum(depth$Vol_m3[c(1:2)])
}

### Concatenate into a single matrix and plot Temp by depth/whole water column
vw_tempf <- cbind.data.frame(vw_temp$DateTime,vw_temp$vw_temp,vw_epi_temp,vw_temp$BVR_3,vw_temp$BVR_4)
...
write_csv(vw_tempf, path = "C:/Users/ahoun/OneDrive/Desktop/GHG/Data_Output/BVR_VW_Temp_16to18_Revs.csv")

### Calculate the same for oxygen
# Re-combine all casts then use spread to convert to wide form
casts_o2 <- rbind(casts_1,casts_2,casts_3,casts_4)
...
for(i in 1:length(casts_layers_o2$Date)){
  vw_o2[i] <- sum(casts_layers_o2$BVR_1[i]*depth$Vol_m3[1],
                    casts_layers_o2$BVR_2[i]*depth$Vol_m3[2],
                    casts_layers_o2$BVR_3[i]*depth$Vol_m3[3],
                    casts_layers_o2$BVR_4[i]*depth$Vol_m3[4])/sum(depth$Vol_m3[c(1:4)])
}
...
for(i in 1:length(casts_layers_o2$Date)){
  vw_epi_o2[i] <- sum(casts_layers_o2$BVR_1[i]*depth$Vol_m3[1],
                        casts_layers_o2$BVR_2[i]*depth$Vol_m3[2])/sum(depth$Vol_m3[c(1:2)])
}

### Concatenate into a single matrix and plot Temp by depth/whole water column
vw_o2f <- cbind.data.frame(vw_o2$DateTime,vw_o2$vw_o2,vw_epi_o2,vw_o2$BVR_3,vw_o2$BVR_4)
...
# Export out Temp Data VW averaged by depth
write_csv(vw_o2f, path = "C:/Users/ahoun/OneDrive/Desktop/GHG/Data_Output/BVR_VW_o2_16to18_Revs.csv")
...
ghg_2_rep1 <- ghg %>% filter(Depth_m==3,Rep==1) %>% group_by(DateTime) %>% 
...
ghg_2_rep2 <- ghg %>% filter(Depth_m==3,Rep==2) %>% group_by(DateTime) %>% 
  summarize_all(funs(mean)) %>% arrange(DateTime) %>% mutate(grouping="BVR_2")
ghg_2_rep2$ch4_umolL <- na.approx(ghg_2_rep2$ch4_umolL)
ghg_2_rep2$co2_umolL <- na.approx(ghg_2_rep2$co2_umolL)

ghg_3_rep1 <- ghg %>% filter(Depth_m==6,Rep==1) %>% group_by(DateTime) %>% 
...
ghg_3_rep2 <- ghg %>% filter(Depth_m==6,Rep==2) %>% group_by(DateTime) %>% 
  summarize_all(funs(mean)) %>% arrange(DateTime) %>% mutate(grouping="BVR_3")
ghg_3_rep2$ch4_umolL <- na.approx(ghg_3_rep2$ch4_umolL)
ghg_3_rep2$co2_umolL <- na.approx(ghg_3_rep2$co2_umolL)

ghg_4_rep1 <- ghg %>% filter(Depth_m==9,Rep==1) %>% group_by(DateTime) %>% 
...
ghg_4_rep2 <- ghg %>% filter(Depth_m==9,Rep==2) %>% group_by(DateTime) %>% 
  summarize_all(funs(mean)) %>% arrange(DateTime) %>% mutate(grouping="BVR_4")
ghg_4_rep2$ch4_umolL <- na.approx(ghg_4_rep2$ch4_umolL)
ghg_4_rep2$co2_umolL <- na.approx(ghg_4_rep2$co2_umolL)

ghg_5_rep1 <- ghg %>% filter(Depth_m==11,Rep==1) %>% group_by(DateTime) %>% 
...
ghg_5_rep2 <- ghg %>% filter(Depth_m==11,Rep==2) %>% group_by(DateTime) %>% 
  summarize_all(funs(mean)) %>% arrange(DateTime) %>% mutate(grouping="BVR_5")
...
for(i in 1:length(rep1_ch4_layers$DateTime)){
  vw_ch4_rep1[i] <- sum(rep1_ch4_layers$BVR_1[i]*depth$Vol_m3[1],
                        rep1_ch4_layers$BVR_2[i]*depth$Vol_m3[2],
                        rep1_ch4_layers$BVR_3[i]*depth$Vol_m3[3],
                        rep1_ch4_layers$BVR_4[i]*depth$Vol_m3[4],
                        rep1_ch4_layers$BVR_5[i]*depth$Vol_m3[5])/sum(depth$Vol_m3[c(1:5)])
}
...
for(i in 1:length(rep2_ch4_layers$DateTime)){
  vw_ch4_rep2[i] <- sum(rep2_ch4_layers$BVR_1[i]*depth$Vol_m3[1],
                        rep2_ch4_layers$BVR_2[i]*depth$Vol_m3[2],
                        rep2_ch4_layers$BVR_3[i]*depth$Vol_m3[3],
                        rep2_ch4_layers$BVR_4[i]*sum(depth$Vol_m3[4],depth$Vol_m3[5]))/sum(depth$Vol_m3[c(1:5)])
}
...
for(i in 1:length(rep1_ch4_layers$DateTime)){
  epi_ch4_rep1[i] <- sum(rep1_ch4_layers$BVR_1[i]*depth$Vol_m3[1],
                         rep1_ch4_layers$BVR_2[i]*depth$Vol_m3[2])/sum(depth$Vol_m3[c(1:2)])
}

# Calculate Epi Rep 2
epi_ch4_rep2 <- rep(-99,length(rep2_ch4_layers$DateTime))
for(i in 1:length(rep2_ch4_layers$DateTime)){
  epi_ch4_rep2[i] <- sum(rep2_ch4_layers$BVR_1[i]*depth$Vol_m3[1],
                         rep2_ch4_layers$BVR_2[i]*depth$Vol_m3[2])/sum(depth$Vol_m3[c(1:2)])
}
...
meta_ch4 <- cbind(rep1_ch4_layers$BVR_3,rep2_ch4_layers$BVR_3)
...
hypo_ch4 <- cbind(vw_ch4_rep1$BVR_4,vw_ch4_rep2$BVR_4)
...
# Export out Temp Data VW averaged by depth
write_csv(vw_ch4_all, path = "C:/Users/ahoun/OneDrive/Desktop/GHG/Data_Output/BVR_VW_ch4_stats_16to18_Revs.csv")
...
for(i in 1:length(rep1_co2_layers$DateTime)){
  vw_co2_rep1[i] <- sum(rep1_co2_layers$BVR_1[i]*depth$Vol_m3[1],
                        rep1_co2_layers$BVR_2[i]*depth$Vol_m3[2],
                        rep1_co2_layers$BVR_3[i]*depth$Vol_m3[4],
                        rep1_co2_layers$BVR_4[i]*sum(depth$Vol_m3[c(4:5)]))/sum(depth$Vol_m3[c(1:5)])
}
...
for(i in 1:length(rep2_co2_layers$DateTime)){
  vw_co2_rep2[i] <- sum(rep2_co2_layers$BVR_1[i]*depth$Vol_m3[1],
                        rep2_co2_layers$BVR_2[i]*depth$Vol_m3[2],
                        rep2_co2_layers$BVR_3[i]*depth$Vol_m3[3],
                        rep2_co2_layers$BVR_4[i]*sum(depth$Vol_m3[c(4:5)]))/sum(depth$Vol_m3[c(1:5)])
}
...
for(i in 1:length(rep1_co2_layers$DateTime)){
  epi_co2_rep1[i] <- sum(rep1_co2_layers$BVR_1[i]*depth$LayerVol_m3[1],
                       rep1_co2_layers$BVR_2[i]*depth$LayerVol_m3[2])/sum(depth$Vol_m3[c(1:2)])
}

# Calculate for Rep 2
epi_co2_rep2 <- rep(-99,length(rep2_co2_layers$DateTime))
for(i in 1:length(rep2_co2_layers$DateTime)){
  epi_co2_rep2[i] <- sum(rep2_co2_layers$BVR_1[i]*depth$Vol_m3[1],
                         rep2_co2_layers$BVR_2[i]*depth$Vol_m3[2])/sum(depth$Vol_m3[c(1:2)])
}
...
meta_co2 <- cbind(rep1_co2_layers$BVR_3,rep2_co2_layers$BVR_3)
...
### USE ONLY 9M
hypo_co2 <- cbind(rep1_co2_layers$BVR_4,rep2_co2_layers$BVR_4)
...
# Export out Temp Data VW averaged by depth
write_csv(vw_co2_all, path = "C:/Users/ahoun/OneDrive/Desktop/GHG/Data_Output/BVR_VW_co2_stats_16to18_Revs.csv")
